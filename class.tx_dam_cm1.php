 ...
 *
 *   44: class tx_dam_cm1
 *  165:     function db_launch($id,$cmd,$label,$icon)
 *  183:     function includeLL()
 *
 * TOTAL FUNCTIONS: 2
			...
			if (is_array($backRef->rec) || $root)	{
				...
				if ($backRef->editOK)	{
					...
					if (!in_array('tx_dam_replace_file',$backRef->disabledItems) AND !$root AND $BE_USER->isPSet($lCP,$table,'editcontent'))	{
						#$icon = $backRef->excludeIcon('<img src="'.$backRef->PH_backPath.PATH_txdam_rel.'i/replace_file.gif'" width="15" height="12" border="0" align="top">');
						$icon = $backRef->excludeIcon('<img'.t3lib_iconWorks::skinImg($backRef->PH_backPath, PATH_txdam_rel.'i/replace_file.gif" width="15" height="12"').' alt="" />');
						$menuItems['tx_dam_replace_file']=$this->db_launch($uid, 'tx_dam_cmd_filereplace', $LANG->getLLL('replaceFile',$LL), $icon);
					}
...
//						$LANG->getLLL('replaceFile',$LL),
//						$backRef->excludeIcon('<img src="'.$backRef->PH_backPath.PATH_txdam_rel.'i/replace_file.gif" width="15" height="12" border=0 align=top>'),
//						$backRef->urlRefForCM($url),
//						1
//					);
				}
