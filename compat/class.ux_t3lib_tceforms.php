 ...
 *
 * $Id: class.t3lib_tceforms.php,v 1.68.2.12 2006/02/15 11:06:18 rfritz Exp $
 ...
 *
 *
 *  197: class ux_t3lib_TCEforms	extends t3lib_TCEforms
 *  302:     function t3lib_TCEforms()
 *  336:     function initDefaultBEmode()
 *
 *              SECTION: Rendering the forms, fields etc
 *  381:     function getSoloField($table,$row,$theFieldToReturn)
 *  420:     function getMainFields($table,$row,$depth=0)
 *  615:     function getListedFields($table,$row,$list)
 *  656:     function getPaletteFields($table,$row,$palette,$header='',$itemList='',$collapsedHeader='')
 *  732:     function getSingleField($table,$field,$row,$altName='',$palette=0,$extra='',$pal=0)
 *  891:     function getSingleField_SW($table,$field,$row,&$PA)
 *
 *              SECTION: Rendering of each TCEform field type
 *  988:     function getSingleField_typeInput($table,$field,$row,&$PA)
 * 1069:     function getSingleField_typeText($table,$field,$row,&$PA)
 * 1181:     function getSingleField_typeCheck($table,$field,$row,&$PA)
 * 1247:     function getSingleField_typeRadio($table,$field,$row,&$PA)
 * 1282:     function getSingleField_typeSelect($table,$field,$row,&$PA)
 * 1362:     function getSingleField_typeSelect_single($table,$field,$row,&$PA,$config,$selItems,$nMV_label)
 * 1479:     function getSingleField_typeSelect_checkbox($table,$field,$row,&$PA,$config,$selItems,$nMV_label)
 * 1598:     function getSingleField_typeSelect_singlebox($table,$field,$row,&$PA,$config,$selItems,$nMV_label)
 * 1708:     function getSingleField_typeSelect_multiple($table,$field,$row,&$PA,$config,$selItems,$nMV_label)
 * 1807:     function getSingleField_typeGroup($table,$field,$row,&$PA)
 * 1976:     function getSingleField_typeNone($table,$field,$row,&$PA)
 * 1992:     function getSingleField_typeNone_render($config,$itemValue)
 * 2054:     function getSingleField_typeFlex($table,$field,$row,&$PA)
 * 2192:     function getSingleField_typeFlex_langMenu($languages,$elName,$selectedLanguage,$multi=1)
 * 2211:     function getSingleField_typeFlex_sheetMenu($sArr,$elName,$sheetKey)
 * 2246:     function getSingleField_typeFlex_draw($dataStruct,$editData,$cmdData,$table,$field,$row,&$PA,$formPrefix='',$level=0,$tRows=array())
 * 2437:     function getSingleField_typeUnknown($table,$field,$row,&$PA)
 * 2452:     function getSingleField_typeUser($table,$field,$row,&$PA)
 *
 *              SECTION: Field content processing
 * 2481:     function formatValue ($config, $itemValue)
 *
 *              SECTION: "Configuration" fetching/processing functions
 * 2573:     function getRTypeNum($table,$row)
 * 2599:     function rearrange($fields)
 * 2625:     function getExcludeElements($table,$row,$typeNum)
 * 2673:     function getFieldsToAdd($table,$row,$typeNum)
 * 2698:     function mergeFieldsWithAddedFields($fields,$fieldsToAdd)
 * 2730:     function setTSconfig($table,$row,$field='')
 * 2752:     function getSpecConfForField($table,$row,$field)
 * 2773:     function getSpecConfFromString($extraString, $defaultExtras)
 *
 *              SECTION: Display of localized content etc.
 * 2801:     function registerDefaultLanguageData($table,$rec)
 * 2833:     function getLanguageOverlayRawValue($table, $row, $field, $fieldConf)
 * 2861:     function renderDefaultLanguageContent($table,$field,$row,$item)
 * 2884:     function renderDefaultLanguageDiff($table,$field,$row,$item)
 *
 *              SECTION: Form element helper functions
 * 2940:     function dbFileIcons($fName,$mode,$allowed,$itemArray,$selector='',$params=array(),$onFocus='')
 * 3093:     function getClipboardElements($allowed,$mode)
 * 3142:     function getClickMenu($str,$table,$uid='')
 * 3163:     function renderWizards($itemKinds,$wizConf,$table,$row,$field,&$PA,$itemName,$specConf,$RTE=0)
 * 3367:     function getIcon($icon)
 * 3394:     function optionTagStyle($iconString)
 * 3410:     function extractValuesOnlyFromValueLabelList($itemFormElValue)
 * 3432:     function wrapOpenPalette($header,$table,$row,$palette,$retFunc=0)
 * 3456:     function checkBoxParams($itemName,$thisValue,$c,$iCount,$addFunc='')
 * 3470:     function elName($itemName)
 * 3481:     function noTitle($str,$wrapParts=array())
 * 3490:     function blur()
 * 3499:     function thisReturnUrl()
 * 3512:     function getSingleHiddenField($table,$field,$row)
 * 3534:     function formWidth($size=48,$textarea=0)
 * 3561:     function formWidthText($size=48,$wrap='')
 * 3577:     function formElStyle($type)
 * 3588:     function formElClass($type)
 * 3599:     function formElStyleClassValue($type, $class=FALSE)
 * 3621:     function insertDefStyle($type)
 * 3640:     function getDynTabMenu($parts, $idString)
 *
 *              SECTION: Item-array manipulation functions (check/select/radio)
 * 3679:     function initItemArray($fieldValue)
 * 3697:     function addItems($items,$iArray)
 * 3719:     function procItems($items,$iArray,$config,$table,$row,$field)
 * 3743:     function addSelectOptionsToItemArray($items,$fieldValue,$TSconfig,$field)
 * 3963:     function addSelectOptionsToItemArray_makeModuleData($value)
 * 3985:     function foreignTable($items,$fieldValue,$TSconfig,$field,$pFFlag=0)
 *
 *              SECTION: Template functions
 * 4065:     function setNewBEDesign()
 * 4120:     function intoTemplate($inArr,$altTemplate='')
 * 4144:     function addUserTemplateMarkers($marker,$table,$field,$row,&$PA)
 * 4155:     function wrapLabels($str)
 * 4168:     function wrapTotal($c,$rec,$table)
 * 4181:     function replaceTableWrap($arr,$rec,$table)
 * 4218:     function wrapBorder(&$out_array,&$out_pointer)
 * 4240:     function rplColorScheme($inTemplate)
 * 4260:     function getDivider()
 * 4270:     function printPalette($palArr)
 * 4321:     function helpTextIcon($table,$field,$force=0)
 * 4341:     function helpText($table,$field)
 * 4362:     function setColorScheme($scheme)
 * 4386:     function resetSchemes()
 * 4397:     function storeSchemes()
 * 4409:     function restoreSchemes()
 *
 *              SECTION: JavaScript related functions
 * 4439:     function JStop()
 * 4490:     function JSbottom($formname='forms[0]')
 * 4804:     function dbFileCon($formObj='document.forms[0]')
 * 5006:     function printNeededJSFunctions()
 * 5033:     function printNeededJSFunctions_top()
 *
 *              SECTION: Various helper functions
 * 5081:     function getDefaultRecord($table,$pid=0)
 * 5120:     function getRecordPath($table,$rec)
 * 5134:     function readPerms()
 * 5148:     function sL($str)
 * 5161:     function getLL($str)
 * 5182:     function isPalettesCollapsed($table,$palette)
 * 5197:     function isDisplayCondition($displayCond,$row,$ffValueKey='')
 * 5298:     function getTSCpid($table,$uid,$pid)
 * 5312:     function doLoadTableDescr($table)
 * 5324:     function getAvailableLanguages($onlyIsoCoded=1,$setDefault=1)
 *
 *
 * 5366: class ux_t3lib_TCEforms_FE extends t3lib_TCEforms
 * 5374:     function wrapLabels($str)
 * 5384:     function printPalette($palArr)
 * 5409:     function setFancyDesign()
 *
 * TOTAL FUNCTIONS: 100
 ...
 * @author	Kasper Skaarhoj <kasperYYYY@typo3.com>
 */
class ux_t3lib_TCEforms_FE extends t3lib_TCEforms {
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/dam/compat/class.ux_t3lib_tceforms.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/dam/compat/class.ux_t3lib_tceforms.php']);
}
?>
