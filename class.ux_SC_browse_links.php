...
#		$GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['dam']['setup']['devel'] = true;
		if (!is_object($this->damSC)) {
			...
			$this->pObj->addParams = t3lib_div::_GET();

			$this->damSC->sl->initSelection_getStored_mergeSubmitted();

//debug($GLOBALS['HTTP_POST_VARS'],'HTTP_POST_VARS');
//debug($GLOBALS['HTTP_GET_VARS'],'HTTP_GET_VARS');
//debug($this->damSC->sl->sel,'damSC->sl->sel');
...
#TODO can be merged with mainf_file
		...
		switch($this->act)	{
			case 'file':
				$content.= $this->dam_select($pArr,!$this->thumbDisplay(),$pArr[4]);
				$content.= $this->thumbNailCheck();
			break;
			case 'upload':
				$content.= $this->dam_upload($pArr,!$this->thumbDisplay(),$pArr[4]);
			break;
		}
...
//debug('main_file');
		...
		$this->expandFolder = $this->expandFolder ? $this->expandFolder : tx_dam_div::getAbsPath($this->damSC->path);
		...
		$this->damSC->path = tx_dam_div::getRelPath($path); // mabe not needed
		...
		$this->act = ($this->act=='file' OR $this->act=='page') ? 'file' : $this->act;
			...
			// Initializing the action value, possibly removing blinded values etc:
		$allowedItems = array_diff(explode(',','file,fileraw,upload'),t3lib_div::trimExplode(',',$this->thisConfig['blindLinkOptions'],1));
		reset($allowedItems);
...
//		if ($BE_USER->getTSConfigVal('options.uploadFieldsInTopOfEB'))	$content.=$uploadForm;
		...
		switch($this->act)	{
			case 'file':
				$content.= $this->dam_select($pArr,!$this->thumbDisplay(),$pArr[3]);
				$content.= $this->thumbNailCheck();
			break;
			case 'fileraw':
				$content.= $this->file_select($pArr,!$this->thumbDisplay());
				$content.= $this->thumbNailCheck($path);
				$content.='<br /><br />';
				if ($BE_USER->isAdmin() || $BE_USER->getTSConfigVal('options.createFoldersInEB'))	$content.= $this->createFolder($path);
			break;
			case 'upload':
				$content.= $this->dam_upload($pArr,!$this->thumbDisplay(),$pArr[4]);
				$content.='<br /><br />';
...
###		$foldertree->ext_noTempRecyclerDirs = ($this->mode == 'filedrag');
		...
		$browseTrees->init('browse_links.php', 'elbrowser');
...
;
				...
				$fileList.= $this->damSC->getResultBrowser();
				$fileList.= '<br/>';
...
#			}
		...
		//
#		$content.= $this->doc->section('',$this->damSC->getCurrentSelectionBox(),0,1);
...
//-----------------------------------------------------------------
	 ...
	 */
 	function dam_getFileListArr ($extensionList, $mode) {
		...
		$this->damSC->execSelectionQuery(TRUE);
		$this->damSC->setSelectionCounter();

			// any records found?
		if($this->damSC->resCountAll) {

				// limit query for browsing
			$this->damSC->addLimitToQuery();
			$this->damSC->execSelectionQuery();

			if($this->damSC->res) {
				while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($this->damSC->res)) {
					...
					$filearray[] = $fI;
					if (count($filearray) >= $this->damSC->resultsPerPage) {
						break;
					}
				}
...
//-----------------------------------------------------------------
	...
	function dam_upload($pArr,$noThumbs,$extensionList='')	{
		...
		if ($path!='/' && @is_dir($path))	{
			...
			$conf['path'] = PATH_thisScript;
			$this->damUL->init($this->damSC, $conf);
...
//-----------------------------------------------------------------
	...
	function thumbNailCheck($path='') {
			// Getting flag for showing/not showing thumbnails:
		$noThumbs = $GLOBALS['BE_USER']->getTSConfigVal('options.noThumbsInEB');

		if (!$noThumbs)	{
			$addParams = t3lib_div::_GET();
			$addParams['act'] = $this->act;
			$addParams['mode'] = $this->mode;
			if($path) {
				$addParams['expandFolder'] = rawurlencode($path);
			}
			$addParams['bparams'] = rawurlencode($this->bparams);
			$thumbNailCheck = t3lib_BEfunc::getFuncCheck('','SET[displayThumbs]',$this->thumbDisplay(),'browse_links.php',t3lib_div::implodeArrayForUrl('',$addParams)).' '.$GLOBALS['LANG']->sL('LLL:EXT:lang/locallang_mod_file_list.php:displayThumbs',1);
		} else {
			$thumbNailCheck='';
		}
	...
	function thumbDisplay() {
			// Getting flag for showing/not showing thumbnails:
		$thumbDisplay = !$GLOBALS['BE_USER']->getTSConfigVal('options.noThumbsInEB');

		if ($thumbDisplay)	{
				// MENU-ITEMS, fetching the setting for thumbnails from File>List module:
			$_MOD_MENU = array('displayThumbs' => '');
			#$_MCONF['name']=$this->damSC->MCONF['name'];
			$_MCONF['name']='file_list';
			$_MOD_SETTINGS = t3lib_BEfunc::getModuleData($_MOD_MENU, t3lib_div::_GP('SET'), $_MCONF['name']);
			$thumbDisplay = $_MOD_SETTINGS['displayThumbs'];
		}
		return $thumbDisplay;
	}
}

if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/dam/class.ux_SC_browse_links.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/dam/class.ux_SC_browse_links.php']);
}
